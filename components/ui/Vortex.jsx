...
/**
 * Vortex — the Originkit "Tornado" particle background, ported to plain JS.
 * Strands spiral up a pinched hyperboloid, dots ride them and flicker, comets
 * race along them — a strike flashes a dot out and ripples everything nearby.
 *
 * House additions over the original: the loop is frozen (one held frame) while
 * the component is offscreen — this sits under the project index, below the
 * live-iframe cinema, and must not spend the page's frame budget when nobody
 * can see it. Reduced motion holds a single static frame the same way. WebGL
 * init failure degrades to a transparent canvas over the CSS ground. §9
 */

/** Whether the reader has asked for less movement. Starts false so the server
...
// The radii read in screen pixels on the panel and world units on the wire.
// The camera frames the form's full height, so at the 600px intrinsic height
// ten world units span the frame — sixty pixels to the unit.
...
const WOBBLE = 0.008;
// Share of each strand's length that fades out at either end.
const FADE_ZONE = 0.15;

// The form's height in world units — the camera is solved to frame this.
const FORM_HEIGHT = 10;

// The zoom the framing is solved for; the prop is mirrored around it so the
// prop reads as a zoom (higher is closer) against FOV's inverse behaviour.
const BASE_ZOOM = 67;
...
// The ripple: a burst shoves every dot inside its radius; position and size
// spring back independently so a dot swells as it is pushed.
...
// The shockwave from a burst — a travelling ring that moves the STRANDS.
const WAVE_SPEED = 5;
...
// The stretch of a strand a comet runs, and the fade at either end.
const RUN_LOW = 0.03;
const RUN_HIGH = 0.95;
const RUN_FADE = 0.1;
...
// Strands rewrite at this rate rather than every frame — 32k vertices at a
// flow this slow reads the same at 30Hz as at 60.
...
/**
 * Monotone cubic interpolation through a set of points. Monotone specifically:
 * the radius profile has a hard pinch at the waist, and an ordinary cubic
 * overshoots either side of a corner like that — the Fritsch-Carlson limiter
 * is what stops the strands bulging back out just before the neck.
 */
...
/**
 * The vortex as three curves against distance along a strand: how far out,
 * how high, how far round. The intermediate radius points are placed as
 * fractions of wherever the waist sits, so moving the waist keeps the profile
 * shape instead of letting control points cross.
 */
function makeShape(cfg) {
  ...
  return {
    ...
    writePoint(out, at, s, lane, flow, wobble, phase, time) {
      ...
      // Sway is a share of the local radius — a fixed distance is nothing
      // at the floor plate and violent at the waist.
...
function createVortex(container, cfgRef) {
  ...
  /* ---------------------------------------- hover repulsion
   * One set of uniforms shared by every material, patched into the stock
   * three materials at compile time so the pointer moves the whole form as
   * one field. */
  const repelUniforms = {
  ...
  const withRepel = (material) => {
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader
        ...
        // Last thing before the vertex is done: the shove is in clip space,
        // a fixed number of screen pixels whatever the depth.
        .replace(
          "#include <fog_vertex>",
          `
          #include <fog_vertex>
  ...
  // re-parsing three of them every frame is work for nothing.
  let lastColors = { line: "", dot: "", comet: "" };

  /** Pick up any colour the config has changed since the last frame. */
  ...
  function build() {
    ...
    for (let i = 0; i < count; i++) {
      // Every strand runs the full height; the spread in speed and phase is
      // what keeps that from reading as one rigid mesh.
      strands.push({
    ...
    for (let i = 0; i < dotCount; i++) {
      // Half crowd the middle of a strand, the rest spread over nearly all.
      const s =
        Math.random() < 0.5 ? 0.2 + Math.random() * 0.4 : 0.05 + Math.random() * 0.9;
  ...
  /* ---------------------------------------- resize */
  // Kept from the last resize so the loop never reads layout per frame.
  let viewHeight = 1;
  ...
  function driveComet(comet, now, dt, entrance) {
    ...
    // Spent once it has run off the far end — which end depends on the way
    // it set off.
    ...
    // both ways — a negative colour is a comet that never appears.
    const ends =
      clamp((comet.s - RUN_LOW) / RUN_FADE, 0, 1) *
      clamp((RUN_HIGH - comet.s) / RUN_FADE, 0, 1);

    for (let i = 0; i < tailLen; i++) {
      // The tail hangs off the back, laid the way the comet came from.
      ...
      // The first few segments run hotter — the head's core.
  ...
  function collide(now) {
    ...
    for (const comet of cometList) {
      ...
      // them against every comet is most of a frame for nothing visible.
      for (let i = 0; i < dotCount; i += 3) {
        ...
        if (sq < hitSq && dotHitAt[i] === 0) {
          // Everything the strike does scales by the one force.
          dotHitAt[i] = 0.001;
  ...
  function step(now) {
    ...
    // Paused (offscreen or reduced motion): one frame, held. The loop keeps
    // ticking so the state can flip back without a rebuild, but no time
    // passes and nothing is laid out or drawn.
    if (!cfg.running) {
      if (!heldFrame) {
        renderer.render(scene, camera);
        heldFrame = true;
    ...
    // built — the entrance plays when the thing is first seen.
    if (born === 0) born = now;
    ...
    // time, so changing any of them does not restart the entrance.
    syncColors(cfg);
    ...
    // `t * speed` would re-read the whole history at the new rate.
    ...
    if (dotMesh && dotCount > 0) {
      ...
      for (let i = 0; i < dotCount; i++) {
        ...
        // The flicker. A sine raised to a power spends most of its time near
        // nothing and spikes briefly — a twinkle, not a throb — and the
        // floor keeps a dot from ever going fully out.
        const beat =
          1 -
          cfg.dotFlicker +
          cfg.dotFlicker *
...
export default function Vortex(props) {
  ...
  // Offscreen, the engine holds a frame and spends nothing — the rootMargin
  // wakes it just before it scrolls in so the entrance is already running.
  ...
  // may pass only part of a group, so each is laid over its defaults.
  ...
  const config = {
    ...
    // Measured DOWN from the top; the shape measures up from the floor.
    waistAt: 1 - waistPosition / 100,
    twist,
    zoom,
    // Direction is the sign on the flow; comets read the sign on its own to
    // decide which end of a strand they start from.
  ...
  // Only the values deciding how many things there are, or the shape they
  // sit on, are in the key — a rebuild lays every buffer out again and
  // restarts the entrance. Everything else is read off the ref per frame.
  ...
  // The first run is skipped: the mount effect has just built the scene from
  // this very config, and building again would restart the entrance.
